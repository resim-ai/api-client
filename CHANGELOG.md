...
- Library consumers: the regenerated client removes the ViewSession surface (no longer served by the deployed API), adds the agents / pool-label-queue methods to `ClientWithResponsesInterface`, and narrows `ListBranchesForProjectParams.OrderBy` from `*string` to a generated enum type.
...
- Updates the output from the slack output to include Running and Cancelled statuses and hide unnecessary statuses.
