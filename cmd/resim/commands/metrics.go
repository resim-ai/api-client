...
const (
	...
	metricsAllowTopicRemovalKey = "allow-topic-removal"
)
...
func init() {
	...
	syncMetricsCmd.Flags().Bool(metricsAllowTopicRemovalKey, false, "Confirm removing any topics this sync would drop. Without this flag, a sync that drops a topic is rejected after previewing the impact.")
...
func syncMetrics(cmd *cobra.Command, args []string) {
	...
	allowTopicRemoval := viper.GetBool(metricsAllowTopicRemovalKey)
	...
	if err := SyncMetricsConfig(projectID, branchID, configPaths, templatesPath, allowTopicRemoval, verboseMode); err != nil {
		log.Fatal(err)
	}
}
