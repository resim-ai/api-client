...
func updateWorkflow(ccmd *cobra.Command, args []string) {
	...
	if viper.IsSet(workflowSuitesKey) || viper.IsSet(workflowSuitesFileKey) {
		// Parse suites JSON
		type suiteSpec struct {
		...
		// Convert to inputs (replace suites set)
		inputs := make([]api.WorkflowSuiteInput, 0, len(specs))
		for _, s := range specs {
			if s.TestSuiteID == "" {
			...
			inputs = append(inputs, api.WorkflowSuiteInput{Enabled: s.Enabled, TestSuiteID: id})
		}
		req.WorkflowSuites = &inputs
	}

	// If nothing set, error out
	if req.Name == nil && req.Description == nil && req.CiWorkflowLink == nil && req.WorkflowSuites == nil {
